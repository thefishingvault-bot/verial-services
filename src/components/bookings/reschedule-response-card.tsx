...
export function RescheduleResponseCard({ bookingId, reschedule }: Props) {
  ...
  const act = (action: "approve" | "decline") => {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) {
        ...
        setError(text || "Failed to update reschedule request");
        return;
      }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Customer requested <span className="font-semibold">{formatDateLabel(new Date(reschedule.proposedDate))}</span>
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {reschedule.customerNote && (
          <div className="rounded-md bg-muted p-3 text-sm text-muted-foreground">
            <p className="font-medium text-foreground">Customer note</p>
            <p>{reschedule.customerNote}</p>
          </div>
        )}
        <div className="space-y-2">
          <Label htmlFor="provider-note">Your note (optional)</Label>
          <Textarea
            id="provider-note"
            value={note}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="flex flex-1 justify-end text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              ...
              Requested {formatDateLabel(new Date(reschedule.createdAt))}
            </div>
          </div>
        </div>
      </CardContent>
